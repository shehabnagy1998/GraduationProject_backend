module.exports = async (req, res, database) => {
  ...
  let isNameExist = async (_) => {
    try {
      ...
      if (res.length >= 1 && courseInfo.name !== name) return true;
      else return false;
    } catch (error) {
      console.log(error);
      errFlag = true;
  ...
  let insertUpdate = async (_) => {
    try {
      const res = await database(
        "UPDATE course SET name=?, total_mark=?, doctor_code=?, department_id=?, grade_year_id=? WHERE code=?",
        [name, total_mark, doctor_code, department_id, grade_year_id, code]
      );
    } catch (error) {
      console.log(error);
      errFlag = true;
