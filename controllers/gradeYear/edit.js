module.exports = async (req, res, database) => {
  ...
  let isExist = async _ => {
    try {
      const res = await database('SELECT * FROM grade_year WHERE id=? LIMIt 1', [id]);
  ...
  let edit = async _ => {
    try {
      const res = await database("UPDATE grade_year SET name=? WHERE id=?", [name, id]);
    } catch (error) {
      console.log(error);
      errFlag = true
    }
  ...
  if (!(await isExist())) {
    res.status(402).send({ msg: `grade year doesn't exist` });
    return
  }
